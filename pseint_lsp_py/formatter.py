...
def format_pseint_code(code_string: str) -> str:
    ...
    all_keywords_list: List[str] = [
        ...
        "Algoritmo", "FinAlgoritmo", "Funcion", "FinFuncion", 
        "Definir", "Como", "Leer", "Escribir", "Escribir Sin Saltar", 
        "Si", "Entonces", "Sino", "FinSi", "Mientras", "Hacer", "FinMientras", 
        "Para", "Hasta", "Con Paso", "FinPara", "Segun", "Caso", 
        "De Otro Modo", "FinSegun", "Repetir", "Hasta Que", "Entero", "Real", 
        "Numero", "Logico", "Booleano", "Caracter", "Texto", "Cadena", 
        "MOD", "Por Referencia" 
    ]
    all_keywords_lower_to_proper_case: Dict[str, str] = {kw.lower(): kw for kw in all_keywords_list}

    # Keywords that start a new indentation level for the lines *following* them
    indent_starters: Set[str] = {
        "proceso", "subproceso", "algoritmo", "funcion", 
        "si", "mientras", "para", "segun", "repetir",
        "sino", "de otro modo", # "caso" is effectively handled by mid_transitions then this for body
    }
    # Keywords that end an indentation level (i.e., they themselves are placed at the outer level)
    indent_enders: Set[str] = {
        "finproceso", "finsubproceso", "finalgoritmo", "finfuncion", 
    ...
    def get_keyword_starting_line(line_content_lower: str, keywords_set: Set[str]) -> Optional[str]:
        """
    ...
    in_repetir_block_awaiting_hasta_que: bool = (
        False 
    )
    ...
    for line_number, line in enumerate(lines):
        ...
        for kw_lower, kw_proper in sorted_keywords_for_casing:
            pattern_parts: List[str] = [re.escape(part) for part in kw_lower.split(' ')]
            regex_pattern: str = r"\b" + r"\s+".join(pattern_parts) + r"\b"
            
            try:
                main_code_part_cased = re.sub(regex_pattern, kw_proper, main_code_part_cased, flags=re.IGNORECASE)
        ...
        original_main_code_part_lower_for_split = original_main_code_part_for_indent_logic.lower()
        ...
        if original_main_code_part_lower_for_split.startswith("caso"):
            ...
            match = re.match(r"^(De Otro Modo:?)(.*)", processed_main_code_for_split) 
            if match:
                keyword_section = match.group(1).strip() 
                if not keyword_section.endswith(":"): 
                    keyword_section += ":"
                trailing_statement = match.group(2).strip()
                if trailing_statement:
                    processed_main_code_for_split = keyword_section
        ...
        matched_ender: Optional[str] = get_keyword_starting_line(effective_code_lower, indent_enders)
        ...
        if potential_split_statement:
            ...
            formatted_lines.append(current_split_line_indent_str + potential_split_statement)
        ...
        if is_current_line_empty_after_strip:
            ...
            for j in range(i + 1, len(formatted_lines)):
                ...
                if next_line_to_check_stripped: # Found next non-blank line
                    ...
                    if get_keyword_starting_line(next_line_to_check_stripped.lower(), indent_enders):
            ...
            last_line_was_blank = True # Mark that we've processed (and possibly added) a blank
        else:
